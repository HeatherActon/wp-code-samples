...
namespace CodeSampleHybridWPTheme\Comments;
...
function comment_navigation( $location = '' ) {
	...
	$output .= '<h3 class="screen-reader-text">' . esc_html__( 'Comment navigation', 'codesample' ) . '</h3>';
	$output .= '<div class="nav-links">';
	$output .= '<div class="nav-previous">' . get_previous_comments_link( esc_html__( 'Older Comments', 'codesample' ) ) . '</div>';
	$output .= '<div class="nav-next">' . get_next_comments_link( esc_html__( 'Newer Comments', 'codesample' ) ) . '</div>';
	$output .= '</div>';
	$output .= '</nav>';
...
function comment_form_button_class( $args ) {
	...
	$args['title_reply']        = __( 'Leave a comment', 'codesample' );
