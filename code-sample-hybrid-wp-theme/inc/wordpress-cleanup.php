...
function be_excerpt_more( $more ) {
	global $post;
	$more = str_replace( '[&hellip;]', '', $more );

	if ( ! is_page_template( 'search.php' ) ) {
		return $more . '<a class="more-link" href="' . get_the_permalink() . '">Learn more ></a>';
	}
	else {
		return $more . '<a class="more-link" href="' . get_the_permalink() . '">Read more ></a>';
	}
}
add_filter( 'the_excerpt', 'be_excerpt_more' );

// This works only for post archives, not core/latest-posts block.
...
// This is needed to trigger the addition of read more text in core/latest-posts block used on home.
function be_excerpt( $excerpt ) {
	if ( ! str_contains( $excerpt, '[&hellip;]' ) ) {
		$excerpt .= ' [&hellip;]';
	}
	return $excerpt;
}
add_filter( 'get_the_excerpt', 'be_excerpt', 10, 1 );

// Modifying the read more text in core/latest-posts block used on home.
function be_text_strings( $translated_text ) {
	switch ( $translated_text ) {

		case '… <a href="%1$s" rel="noopener noreferrer">Read more<span class="screen-reader-text">: %2$s</span></a>':
			$translated_text = ' <a href="%1$s" rel="noopener noreferrer">Learn more ><span class="screen-reader-text">: %2$s</span></a>';
			break;

	}
	return $translated_text;
}
add_filter( 'gettext', 'be_text_strings', 20, 1 );
