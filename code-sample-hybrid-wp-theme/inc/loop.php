...
/**
 * Recipe Loop - triggered in archive-recipe.php
 */
function be_recipe_loop() {
	...
	if ( have_posts() ) :
		...
		while ( have_posts() && ! is_404() ) :
			the_post();
			tha_entry_before();
			$partial = is_singular() ? 'content' : 'archive-recipe';
			get_template_part( 'partials/content/' . $partial );
			tha_entry_after();
		endwhile;
...
/**
 * Story Loop - triggered in archive-story.php
 */
function be_story_loop() {
	...
	if ( have_posts() ) :
		...
		while ( have_posts() && ! is_404() ) :
			the_post();
			tha_entry_before();
			$partial = is_singular() ? 'content' : 'archive-story';
			get_template_part( 'partials/content/' . $partial );
			tha_entry_after();
		endwhile;
...
function codesample_order_archive_cpts( $query ) {
...
add_action( 'pre_get_posts', 'codesample_order_archive_cpts' );
